...
export default function Home(today: any){
  ...
  useEffect(()=> {
    ...
    axios.get(`https://api.nasa.gov/planetary/apod?api_key=${apiKey}&date=${date}`).then(data => setData(data)).catch(error => setError(error.toJSON()))
  }, [date])
...
if(error){
  ...
  return data ? <div className="container">
      ...
      <img className="universe-img" src={data.data.url} alt="..."/> : data.data.media_type === "video" ? <iframe className="youtube-video" src={data.data.url} title="Apod" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen></iframe> :
     <p className="no-img">Sorry, the media you are searching for is not found. Please pick another date to see the photo.</p>}
      <div className="description">
        <h2 className="img-title">{data.data.title}</h2>
